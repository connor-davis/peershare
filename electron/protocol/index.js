...
class Protocol {
  ...
  async createSwarm(domain, reply = (event, data) => {}) {
    ...
    this.swarm.on('connection', (connection, information) => {
      ...
      this.events.subscribe((event) => {
        if (event.type === 'get-remote-files')
        ...
        if (event.type === 'local-message') {
          ...
          connection.write(
            JSON.stringify({ type: 'remote-message', content: event.content })
          );
        }
        ...
        if (event.type === 'local-file-added')
          return connection.write(
            JSON.stringify({
              ...event,
        ...
        if (event.type === 'local-file-removed')
          return connection.write(
            JSON.stringify({
              ...event,
        ...
        if (event.type === 'remote-file-added')
          return this.addRemoteFile(event.file, reply);
        if (event.type === 'remote-file-removed')
          return this.removeRemoteFile(event.file, reply);
        ...
        if (event.type === 'download-file')
          return connection.write(
            JSON.stringify({
              ...
              key: event.key,
              owner: event.owner,
              requester: Buffer.from(this.swarm.keyPair.publicKey).toString(
                'hex'
              ),
            })
          );
        ...
        if (
          ...
          node.events.subscribe((nodeEvent) => {
            ...
            if (nodeEvent.type === 'file-uploaded')
        ...
        if (
          ...
          node.events.subscribe((nodeEvent) => {
            ...
            if (nodeEvent.type === 'file-downloaded')
              return reply('complete', {
                key,
                upload: false,
                download: true,
  ...
  addLocalFile(file, reply = (event, data) => {}) {
    file = {
      ...file,
      owner: Buffer.from(this.swarm.keyPair.publicKey).toString('hex'),
    };
